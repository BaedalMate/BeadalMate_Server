...
public interface RecruitJpaRepository extends JpaRepository<Recruit, Long>, RecruitCustomRepository {
    ...
    @Modifying(clearAutomatically = true)
    @Query("update Recruit r set r.active = false, r.deactiveDate = :date where r.id = :id")
    void setActiveFalse(@Param("id") Long recruitId, @Param("date") LocalDateTime date);

    @Modifying(clearAutomatically = true)
    @Query("update Recruit r set r.cancel = true, r.active = false, r.deactiveDate = :date where r.id = :id")
    void setCancelTrueAndActiveFalse(@Param("id") Long recruitId, @Param("date") LocalDateTime date);

    @Modifying(clearAutomatically = true)
    @Query("update Recruit r set r.active = false, r.deactiveDate = :date  " +
    ...
    @Modifying(clearAutomatically = true)
    @Query("update Recruit r set r.active = false, r.fail = true, r.deactiveDate = :date  " +
            "where r.active = true and r.cancel = false and r.fail = false and r.criteria != baedalmate.baedalmate.recruit.domain.Criteria.TIME and r.deadlineDate < :date")
