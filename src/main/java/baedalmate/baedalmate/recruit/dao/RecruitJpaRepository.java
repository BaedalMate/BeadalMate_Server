...
public interface RecruitJpaRepository extends JpaRepository<Recruit, Long> {
    ...
    @Modifying(clearAutomatically = true)
    @Query("update Recruit r set r.active = false, r.cancel = true where r.active = true and r.cancel = false and r.criteria is not TIME and r.deadlineDate < :date")
    void setCancelTrueFromRecruitExceedTime(@Param("date") LocalDateTime date);

    @Modifying(clearAutomatically = true)
    @Query("update Recruit r set r.active = false where r.active = true and r.cancel = false and r.criteria is TIME and r.deadlineDate < :date")
